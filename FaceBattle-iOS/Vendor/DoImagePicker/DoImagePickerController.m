...
- (void)viewDidLoad
{
    ...
    UINib *nib2 = [UINib nibWithNibName:@"DoAlbumThumbCell" bundle:nil];
    [_tvAlblumCollectionView registerNib:nib2 forCellWithReuseIdentifier:@"DoAlbumThumbCell"];
...
//    _tvAlbumList.alpha = 0.0;
    ...
    _bottomView.frame = CGRectMake(0, self.view.frame.size.height - _bottomView.frame.size.height,
                                   self.view.frame.size.width, _bottomView.frame.size.height);
    
    _tvAlblumCollectionView.frame = CGRectMake(0, _vBottomMenu.frame.origin.y +
                                               _vBottomMenu.frame.size.height,
                                               self.view.frame.size.width,
                                               0);
    _tvAlblumCollectionView.alpha = 0.0;
...
/*- (void)readAlbumList:(BOOL)bFirst
{
    [ASSETHELPER getGroupList:^(NSArray *aGroups) {
        ...
        float albumListHeight = MIN(aGroups.count * 50, 200);
        _tvAlbumList.frame = CGRectMake(_tvAlbumList.frame.origin.x,
                                        _tvAlbumList.frame.origin.y - albumListHeight,
                                        _tvAlbumList.frame.size.width, albumListHeight);
    }];
}*/

//update by collection view
- (void)readAlbumList:(BOOL)bFirst{
    [ASSETHELPER getGroupList:^(NSArray *aGroups) {
        
        [_tvAlblumCollectionView reloadData];
        ...
        if (nIndex < 0)
            nIndex = 0;
#endif
        [_tvAlblumCollectionView selectItemAtIndexPath:
         [NSIndexPath indexPathForRow:nIndex inSection:0]
                                              animated:NO
...
- (IBAction)onSelectAlbum:(id)sender{
    CGRect willChangeFrame = CGRectMake(0, _vBottomMenu.frame.origin.y +
                                        _vBottomMenu.frame.size.height,
                                        _tvAlblumCollectionView.frame.size.width,
                                        self.view.frame.size.height -
                                        _vBottomMenu.frame.origin.y - 44);

    if (_tvAlblumCollectionView.frame.size.height == 0){
        // show tableview
        [UIView animateWithDuration:0.2 animations:^(void) {
            _vDimmed.alpha = 0.7;
            _tvAlblumCollectionView.frame = willChangeFrame;
            _tvAlblumCollectionView.alpha = 1.0;
...
- (void)hideBottomMenu
{
    [UIView animateWithDuration:0.2 animations:^(void) {
        ...
        _tvAlblumCollectionView.frame = CGRectMake(0, _vBottomMenu.frame.origin.y,
                                        _tvAlblumCollectionView.frame.size.width, 0);
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    if ([collectionView isEqual:_tvAlblumCollectionView]) {
        return [ASSETHELPER getGroupCount];
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (collectionView == _tvAlblumCollectionView) {
        DoAlbumThumbCell *cell = (DoAlbumThumbCell *)[_tvAlblumCollectionView
                                                      dequeueReusableCellWithReuseIdentifier:@"DoAlbumThumbCell"
                                                      forIndexPath:indexPath];
        NSDictionary *d = [ASSETHELPER getGroupInfo:indexPath.row];
        NSString *name  = d[@"name"];
        NSString *count = [NSString stringWithFormat:@"%@", d[@"count"]];
        [ASSETHELPER getPhotoLastOneOfGroupByIndex:indexPath.row result:^(UIImage *image) {
...
//        [d[@"image"] getValue:&retrievedCGImageRef];
//        cell.imageView.image = [UIImage imageWithCGImage:retrievedCGImageRef];
        cell.nameLabel.text = [NSString stringWithFormat:@"%@(%@)",name,count];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (collectionView == _tvAlblumCollectionView) {
        return CGSizeMake(104, 150);
    }
    
    //=========================================
//     320 / 3.0 =
    CGSize cellSize = CGSizeMake(SCREEN_WIDTH / _nColumnCount - _nColumnCount,
                                 SCREEN_WIDTH / _nColumnCount - _nColumnCount);
    return cellSize;
...
- (void)hidePreview
{
    [self.view bringSubviewToFront:_tvAlblumCollectionView];
