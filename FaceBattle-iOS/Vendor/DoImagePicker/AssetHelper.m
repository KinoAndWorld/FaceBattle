...
//    _assetPhotos = [[NSMutableArray alloc] init];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [_assetGroups[nGroupIndex] setAssetsFilter:[ALAssetsFilter allPhotos]];
        //    ALAssetsGroup
        UIImage *image = nil;
        if ([_assetGroups[nGroupIndex] numberOfAssets] > 0) {
            image = [UIImage imageWithCGImage:[_assetGroups[nGroupIndex] posterImage]];
        }
        result(image);
    });
